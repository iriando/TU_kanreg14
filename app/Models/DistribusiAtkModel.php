...
class DistribusiAtkModel extends Model
{
    protected $table            = 'log_distribusiatk';
    ...
    public function createDistribusi(array $data, string $petugas)
    {
        ...
        $atk      = $atkModel->getByKode($data['kode_barang']);
        if (!$atk) {
        ...
        if ($this->adjustAtkStock($data['kode_barang'], +$row['jumlah']) === false) {
            $this->db->transRollback();
            return false;
        }
    ...
    public function updateDistribusi(int $id, array $data, string $petugas)
    {
        ...
        // Ambil ATK untuk kode barang baru
        $atkBaru = $atkModel->getByKode($data['kode_barang']);
        if (!$atkBaru) {
            $this->errors = ['kode_barang' => 'Kode barang tidak ditemukan.'];
            return false;
        }
        ...
        $updateRow = [
            'nama_penerima'      => $data['nama_penerima'],
            'kode_barang'        => $data['kode_barang'],
            'nama_barang'        => $atkBaru->nama_barang,
            'jumlah'             => (int) $data['jumlah'],
        ...
        if ($current->kode_barang === $updateRow['kode_barang']) {
            ...
            if ($delta !== 0 && $this->adjustAtkStock($updateRow['kode_barang'], $delta) === false) {
                $this->db->transRollback();
                return false;
            }
        } else {
            ...
            if ($this->adjustAtkStock($current->kode_barang, -(int) $current->jumlah) === false) {
                $this->db->transRollback();
                return false;
            }
            // 2) kurangi stok barang baru
            if ($this->adjustAtkStock($updateRow['kode_barang'], +(int) $updateRow['jumlah']) === false) {
                $this->db->transRollback();
                return false;
            }
        }
    ...
    public function deleteDistribusi(int $id)
    {
        ...
        if ($this->adjustAtkStock($row->kode_barang, -(int) $row->jumlah) === false) {
            $this->db->transRollback();
            return false;
        }
    ...
    /**
     * Menyesuaikan stok ATK berbasis delta distribusi.
     * delta > 0  => tambah didistribusi, kurangi sisa
     * delta < 0  => kurangi didistribusi, tambah sisa (mis. delete / pengurangan)
     */
    private function adjustAtkStock(string $kodeBarang, int $delta)
    {
        $atkModel = new \App\Models\AtkModel();
        $atk      = $atkModel->getByKode($kodeBarang);
        if (!$atk) {
            $this->errors = ['kode_barang' => 'Kode barang ATK tidak ditemukan saat sinkron stok.'];
            return false;
        }

        // Hitung berbasis angka dasar untuk akurasi
        $didistribusiBaru = (int) $atk->didistribusi + $delta;
        if ($didistribusiBaru < 0) {
            $didistribusiBaru = 0; // guard
        }

        // sisa = jumlah awal - didistribusi
        $sisaBaru = (int) $atk->jumlah - $didistribusiBaru;
        if ($sisaBaru < 0) {
            $sisaBaru = 0; // guard, jangan minus
        }

        return $atkModel->updateByKode($kodeBarang, [
